function [vars_out] = Baltic_ferrybox_BEC_colocations_stats(fn_in)
%
% Syntax (function):
% [vars_out] = Baltic_ferrybox_BEC_colocations_stats(fn_in)
%
% Description
% Compute Statistics of the ref-2-Satellite colocations
% whithin a searching distance and within a time window (i.e. ±7-days).
%
% The output from this script is to feed into Baltic_argo_BEC_analyses.m
% This script save colocations [Baltic_argo_BEC_colocations_VARS2SAVE.m]
%
% Argo files were prepared by BEC for validation SSS (Baltic+ and Baltic+)
%
%
% Input
% fn_in: filename with the Colocations data (file created with function
% Baltic_ref_BEC_colocations.m)
%
%
% Output
...
%         embeded into Baltic_argo_BEC_colocations
...
%     'SSS/' basin_str...
%     '/BEC/Validation/indata/ferrybox/Colocations/' version_str '/'];
...
if fn_exist ~= 2
    ...
    'lon_irange','lat_irange','time_irange', 'SALT_ref'}; % it is missing region and basin_str (neeed to be added to colocation files)
    ...
    for ncount = 1:length(PRES_irange(1,:))
        SALT_ref_beta = SALT_irange(find(PRES_irange(:,ncount)<=depth_ref),ncount);
        ...
        if ~isnan(SALT_ref_beta)
            ...
            S_beta = SALT_irange(:,ncount);
            P_beta = PRES_irange(:,ncount);
    ...
    if ~isempty (SALT_ref)
        ...
        disp({'New file with ref-2-satellite colocations '; fn_out})
