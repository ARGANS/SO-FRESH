function [vars_out] = Baltic_ferrybox_BEC_colocations_v3r0(iyear,imonth,iregion,iships,idata_type,varargin)
%
%
% Syntax (function):
...
folder_figs = [folder_figs basin_str '/' REF_str '/ferrybox2SAT/'];
...
% =============================
%* Keep "ferrybox good data" only (flag = 1)
if flag_on == 1
    flag_good = 0;
    
    ind = SSS_QC ~= flag_good &...
        SST_QC ~= flag_good & PRES_QC ~= flag_good;
    
    SALT(ind) = NaN; TEMP(ind) = NaN;
    
end; clear ind flag_good
...
for yy = 1:length(iyear)
    ...
    for mm = 1:length(imonth)
        ...
        % =============================
        % fn_out (.mat)
        fn_out = [folder_out basin_str '_' ships_str '_'...
            REF_str '_R' r_str '_'...
        ...
        run Baltic_ferrybox_BEC_colocations_VARS2SAVE.m % -  snippet -            
            ...
            for dd = 1: length(idays)
                ...
                %* >>>>>>>>>>>>>>>>>>>>
                %* load REF binned data as created using mat script 
                %*(for more info, type Baltic_SDN_BEC_binning_v1r0.m)
                if bin_data == 1
                    
                    % folder name with ferrybox binned data
                    folder_in = [folder_data...
                        REF_str '_mat' '/BINNED/' flag_str '/'...
                        ships_str '/monthly/'];
...
%                                                 lat(ind) = NaN;
                ...
                if ismember(itime_cnt,time_number) == 1
                    ...
                    if ~isempty(lon_alfa) && ~isempty(lat_alfa) && sum(any(SALT_alfa))~=0
                        ...
                        if idata_type == 4 && itime_cnt >= datenum(2011,02,05) && itime_cnt <= datenum(2013,12,27)
                        ...
                        elseif idata_type == 4 && (itime_cnt < datenum(2011,02,05) || itime_cnt > datenum(2013,12,27))
                            sss = nan;
                            lon_sss = NaN;
                            lat_sss = NaN;
                        end
                        ...
                        if idata_type == 5 && itime_cnt >= datenum(2011,02,05) && itime_cnt <= datenum(2013,12,27)
                        ...
                        elseif idata_type == 5 && (itime_cnt < datenum(2011,02,05) || itime_cnt > datenum(2013,12,27))
                            sss = nan;
                            lon_sss = NaN;
                            lat_sss = NaN;
                        end
                        ...
                        for nn = 1: nprof
                            ...
                            if Ln >= 1
                                ...
                                %% [2.3] REF Vertical profile interpolation
                                [a,b] = size(SALT_alfa);
                                ...
                                elseif bin_data == 1
                                PRES_irange (ncount) = PRES_alfa(nn);
                                SALT_irange (ncount) = SALT_alfa(nn);
                                TEMP_irange (ncount) = TEMP_alfa(nn);
                                end
