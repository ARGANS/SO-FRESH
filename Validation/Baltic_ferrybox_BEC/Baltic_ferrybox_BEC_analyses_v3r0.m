% Syntax: Baltic_ferrybox_BEC_analyses_v3r9.m (script)
%
% Description
...
%          [5] Transpaper *
%          [6] Victoria
% [*] shows ship transects with data within 2011-2013 period.
%
...
% [1] Colocations Argo-2-satellite and Argo-2-Model grid/time [Baltic_ferrybox_colocations.m]
...
%
%
% current version: v3r9 (2020/08/15) - NEW VERSION
% - use binned data and flags on/of
%
% History
% [4] add stats and plots v2r1 (2020/03/05)
...
iships = 5;     % [2] Finnamaid, [3] Romantika and [5] Transpaper check options (type help Baltic_ferrybox_BEC_analyses_v2r1.m)
iyear = 2011:2013;
imonth = 1:12;
...
folder_data = ([path_root ...
    'SSS/' basin_str '/BEC/Validation/indata/' REF_str '/']);
...
folder_colocations = [path_root...
    'SSS/Baltic/BEC/Validation/indata/'...
    REF_str '/Colocations/' bin_str '/' flag_str '/cat_files/' ];

% ==========================


% filename Argo BEC structure: ferrybox_20110101_20110110.nc
ndays = 9; % number of days contained in each BEC Argo file

folder_figs = [folder_figs basin_str '/'...
    REF_str '/' bin_str '/' flag_str '/'];
...
% Make output for each SHIP transect
folder_figs = [folder_figs ships_str '/'];
...
%% [1] Colocation Argo-2-Satellite (% Baltic_ferrybox_BEC_colocations.m)
% run ([basin_str '_ferrybox_BEC_colocations.m'])% (snippet)
...
fn_col1 = [folder_colocations version_col1 '/'...
    basin_str '_' ships_str '_ferrybox_R' num2str(r)...
...
title({...
    ...
    ' boxes ' basin_str ' (region ' region '-' ships_str ') '];...
    [time_str1 '-' time_str2]
...
fg_name = ['ferrybox_'...
    ...
    '_BINNED_COUNTS_' region '_' ships_str '_R' num2str(r) 'KM.'  fg_format];
...
fg_name = ['ferrybox_'...
    time_str1 '_' time_str2...
    '_BINNED_MEAN_' region '_' ships_str '_R' num2str(r) 'KM.'  fg_format];
...
fg_name = ['ferrybox_'...
    time_str1 '_' time_str2...
    '_BINNED_MEDIAN_' region '_' ships_str '_R' num2str(r) 'KM.'  fg_format];
...
fg_name = ['ferrybox_'...
    time_str1 '_' time_str2...
    '_BINNED_STD_' region '_' ships_str '_R' num2str(r) 'KM.'  fg_format];
...
if make_plot == 1
    ...
    title({...
        [basin_str ' (region ' region ' - ' ships_str ') '...
    ...
    fg_name = ['ferrybox_'...
        ...
        region '_' ships_str '_R' num2str(r) 'KM.'  fg_format];
...
% %     X4_beta = nanmedian(X4.sss_irange,1);
   ...
   set(findobj(gca,'type','line'),'linew',2) 
    ...
    fg_name = ['ferrybox_' time_str1 '_' time_str2...
        '_BOXPLOT_SSS_' region '_' ships_str...
        '_R' num2str(r) 'KM.'...
        fg_format];
...
%* Use     
%     X1_beta = nanmedian(X1.sss_irange,1) - X1.SALT_ref;
%     X2_beta = nanmedian(X2.sss_irange,1) - X2.SALT_ref;
%     X3_beta = nanmedian(X3.sss_irange,1) - X3.SALT_ref;
%     X4_beta = nanmedian(X4.sss_irange,1) - X4.SALT_ref;
    ...
    fg_name = ['ferrybox_' time_str1 '_' time_str2...
        '_BOXPLOT_dSSS_' region '_' ships_str...
        '_R' num2str(r) 'KM.'...
        fg_format];
...
if CORRECT == 1
    ...
    ind_a = abs(a_beta)>SSS_max | abs(a_beta)<=SSS_min;
    ind_b = abs(b_beta)>SSS_max | abs(b_beta)<=SSS_min;
    ind_c = abs(c_beta)>SSS_max | abs(c_beta)<=SSS_min;
    ind_d = abs(d_beta)>SSS_max | abs(d_beta)<=SSS_min;
...
title({['Stats for SSS ' version_str1_legend ],...
    [basin_str ' ' region '-' ships_str]},...
    'fontsize',24,'interpreter','none');

xlabel('SSS ','fontsize',24);
...
title({['Stats for SSS ' version_str2_legend ],...
    [basin_str ' ' region '-' ships_str]},...
    'fontsize',24,'interpreter','none');
...
title({['Stats for SSS ' version_str3_legend ],...
    [basin_str ' ' region '-' ships_str]},...
    'fontsize',24,'interpreter','none');
...
title({['Stats for SSS ' version_str4_legend ],...
    [basin_str ' ' region '-' ships_str]},...
    'fontsize',24,'interpreter','none');
...
fg_name = ['ferrybox_'...
    datestr(time_a(1),'yyyymmdd') '_'....
    datestr(time_a(end),'yyyymmdd') '_STATS_SSS_'...
    region '_' ships_str '_'...
    'R' num2str(r) 'KM.xlsx'];

% Save figure  - output -
folder_this = [folder_figs 'HISTOGRAMS/SSS/STATS/' CORRECT_str '/'];
...
if CORRECT == 1
    ...
    CORRECT_str = 'FILTERED';
%* Use filter (ind) crearted in the previous section [2.4.1.5]    
     dSSS_max = 8;
%     
...
title({['Stats for dSSS ' version_str4_legend ],...
    [basin_str ' ' region '-' ships_str]},...
    'fontsize',24,'interpreter','none');
...
fg_name = ['ferrybox_'...
    datestr(time_a(1),'yyyymmdd') '_'....
    datestr(time_a(end),'yyyymmdd') '_STATS_dSSS_'...
    region '_' ships_str '_'...
    'R' num2str(r) 'KM.xlsx'];


% Save figure  - output -
folder_this = [folder_figs 'HISTOGRAMS/dSSS/STATS/' CORRECT_str '/'];
...
% Get the one singe reference dataset (i.e. Argo, ferrybox, etc.)
...
fg_name = ['ferrybox_'...
    ...
    '_TAYLOR_SSS_' region '_' ships_str...
