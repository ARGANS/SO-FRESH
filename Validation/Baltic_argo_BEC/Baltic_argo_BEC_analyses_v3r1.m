...
%
% current version: 
% v3r1 (2020/08/21) - NEW VERSION - 
% --> Add option to make to include BEC products only, or also include
% CCI+SSS
%
% History
% [5] v3r0 (2020/07/28) Use BINNED reference data collocated 
% to the center of nearest satellite grid node.
% [4] v2r2 (2020/03/05) - Update plot specs
% [3] v2r1 [yyyy/mm/dd] -
...
REF_str = 'argo'; %input the string of the reference dataset (eg. SDN, ARGO, TARA, etc.)

% ======
%* include all sat product available (sat_prod = 1), 
%* or only BEC product (sat_prod = 0) - excludes CCI+SSS [NEW, 20200821]
prod_all = 0;
...
folder_data = ([path_root ...
   ...
   folder_data = ([path_root ...
    'SSS/' basin_str '/BEC/Validation/indata/Argo/argo_mat/' bin_str '/']);
...
folder_colocations = [path_root...
    'SSS/' basin_str '/BEC/Validation/indata/argo/Colocations/'...
    bin_str '/cat_files/' ];

% filename Argo BEC structure: argo_20110101_20110110.nc
ndays = 9; % number of days contained in each BEC Argo file

folder_figs = [folder_figs basin_str '/argo/Argo-BEC/' bin_str '/'];


if prod_all == 0 
    folder_figs = [folder_figs 'BEC_ONLY/'];
...
elseif prod_all == 0
    time_alfa = [itime1 itime2 itime4];
end
...
ncoloc1_total = sum(Y_beta1);
ncoloc2_total = sum(Y_beta2);
ncoloc3_total = sum(Y_beta3);
ncoloc4_total = sum(Y_beta4);
...
%         [' ' version_str3_legend],[' ' version_str4_legend]};
    ...
    X1_beta = X1.sss_irange_md;
    X2_beta = X2.sss_irange_md;
    if prod_all == 1
    X3_beta = X3.sss_irange_md;
    elseif prod_all == 0
    X3_beta = NaN;
    end
    
    X4_beta = X4.sss_irange_md;
...
%     X4_beta = nanmedian(X4.sss_irange,1);
    ...
    ind1 = X1_beta == 0 | isnan(X1.SALT_ref(:));
    ind2 = X2_beta == 0 | isnan(X2.SALT_ref(:));
    if prod_all == 1
    ind3 = X3_beta == 0 | isnan(X3.SALT_ref(:));
    X3_beta(ind3) = NaN;
    
    end
    ind4 = X4_beta == 0 | isnan(X4.SALT_ref(:));
    ...
    elseif prod_all == 0
    bplot = boxplot(vertcat(C{:}),grp,'labels',PRODS(1,1:end-1));
    ...
    elseif prod_all == 0
        text(2.95,ylim_alfa(1)+1.2,STATS_str{4},'BackgroundColor','w','EdgeColor','k','fontsize',20)
...
ind3 = c_beta == 0 | isnan(X3.SALT_ref(:));
c_beta(ind3) = NaN;
c_beta(isnan(c_beta)) = [];
...
% ============================
% Do not include data with zeros or refenrence data with nan
ind1 = a_beta == 0 | isnan(X1.SALT_ref(:));
ind2 = b_beta == 0 | isnan(X2.SALT_ref(:));
ind4 = d_beta == 0 | isnan(X4.SALT_ref(:));
...
if ~isempty(c_beta) && ~isnan(c_beta)
[X3_mean,X3_median,X3_std,X3_stdRobust,X3_min,X3_max,...
...
if fg_save == 1 && fg_exist == 0
    ...
    n_decimals = 4; % number of decimals display in table
    
    if prod_all == 1
    X_version = {X1.version_str; X2.version_str; X3.version_str; X4.version_str};

    X_nobs = [X1_nobs; X2_nobs; X3_nobs; X4_nobs];
    X_mean = round([X1_mean;X2_mean;X3_mean;X4_mean],n_decimals,'decimals');
    X_median = round([X1_median;X2_median;X3_median;X4_median],n_decimals,'decimals');
    X_std = round([X1_std;X2_std;X3_std;X4_std],n_decimals,'decimals');
    X_stdRobust = round([X1_stdRobust;X2_stdRobust;X3_stdRobust;X4_stdRobust],n_decimals,'decimals');
    X_min = round([X1_min;X2_min;X3_min;X4_min],n_decimals,'decimals');
    X_max = round([X1_max;X2_max;X3_max;X4_max],n_decimals,'decimals');
    X_SEM = round([X1_SEM;X2_SEM;X3_SEM;X4_SEM],n_decimals,'decimals');
    X_Q2 = round([X1_Q2;X2_Q2;X3_Q2;X4_Q2],n_decimals,'decimals');
    X_Q4 = round([X1_Q4;X2_Q4;X3_Q4;X4_Q4],n_decimals,'decimals');
    X_IQR = round([X1_IQR;X2_IQR;X3_IQR;X4_IQR],n_decimals,'decimals');
    
    elseif prod_all == 0
        
    X_version = {X1.version_str; X2.version_str; X4.version_str};

    X_nobs = [X1_nobs; X2_nobs; X4_nobs];
    X_mean = round([X1_mean;X2_mean;X4_mean],n_decimals,'decimals');
    X_median = round([X1_median;X2_median;X4_median],n_decimals,'decimals');
    X_std = round([X1_std;X2_std;X4_std],n_decimals,'decimals');
    X_stdRobust = round([X1_stdRobust;X2_stdRobust;X4_stdRobust],n_decimals,'decimals');
    X_min = round([X1_min;X2_min;X4_min],n_decimals,'decimals');
    X_max = round([X1_max;X2_max;X4_max],n_decimals,'decimals');
    X_SEM = round([X1_SEM;X2_SEM;X4_SEM],n_decimals,'decimals');
    X_Q2 = round([X1_Q2;X2_Q2;X4_Q2],n_decimals,'decimals');
    X_Q4 = round([X1_Q4;X2_Q4;X4_Q4],n_decimals,'decimals');
    X_IQR = round([X1_IQR;X2_IQR;X4_IQR],n_decimals,'decimals');
...
elseif prod_all == 0
time_max = max([time_a time_b time_d]);
time_min = min([time_a time_b time_d]);
end
...
%* BUG: legend wrongly displayed when there are NaN elements (FIXED: remove
%* NaN before adding legend).
ind = find(~isnan(CORR_ALL(2:end)));


lg1 = legend(hp2,PRODS(ind));
