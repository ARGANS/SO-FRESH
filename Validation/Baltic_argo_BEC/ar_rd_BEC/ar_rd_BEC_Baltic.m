...
for yy = 1:length(iYEARS)
    ...
    for mm = 1: length(iMONTHS)
        ...
        for dd = 1 : num_days
            ...
            % =========
            % if there is netCDF file and not a matFIle then create it
             exist_fn_in = exist(fn_in,'file');
             exist_fn_out = exist(fn_out,'file');
            
            if exist_fn_in == 2 && exist_fn_out ~= 2
                % Make a folder_out, ONLY if there is Argo data
