function [vars_out] = Baltic_argo_BEC_colocations_v3r0(iyear,imonth,iregion,idata_type)
%
% Syntax (function):
...
%
%
% current version: v3r0 (2020/07/27) -
%                   [1] use binned argo data with the same spatial and
%                   temporal resolution as satellite data.
%
% History
% v2r0 (2020/03/18) - convert script into function
...
% Path to binned data
if bin_data == 1
    folder_data = ([folder_data bin_str '/']);
end
%% Save Colocation output
folder_out = [path_root...
    'SSS/' basin_str...
    '/BEC/Validation/indata/argo/Colocations/' bin_str '/monthly/'...
    version_str '/'];
...
if bin_data == 1
    save_vars_out = [argo_vars_out sat_vars_out colocated_vars_out 'count_bin'];
    ...
    for mm = 1:length(imonth)
        ...
        if fn_out_exist ~= 2 || run_test == 1
            ...
            for dd = 1:length(idays)
                ...
                if bin_data == 0
                    time_end_str = datestr(itime_end+1,'yyyymmdd');
                ...
                elseif bin_data == 1
                    ...
                    % load ARGO-matlab BINNED file [fn]
                ...
                if fn_exist == 2
                    ...
                    if bin_data == 0
                        %* if not-binned data, simply load data in workspace
                        load (fn_in)
                        SALT = PSAL;
                    elseif bin_data == 1
                        %* if binned data, load data as struct (stored in TT)
                        %* in workspace
                        TT = load(fn_in);
                        lon =  TT.lon_bin;
                        lat = TT.lat_bin;
                        ...
                        % convert size itime to same size as lon lat
                        time_cnt = ones(size(lon)).*time_cnt;
                        JULD = ones(size(lon)).*JULD;
                        % ====================
                        
                        if size(lon) ~= size(SALT)
                            disp(['number of salt values should'...
                                'be the same as lon'])
                        end
                    ...
                    if exist('lon','var') && exist('lat','var')
                        ...
                        if ~isempty(lon) && ~isempty(lat)
                            ...
                            if idata_type == 4 && itime_cnt <= datenum(2013,12,27)
                            ...
                            elseif idata_type == 1 && itime_cnt > datenum(2019,08,31)
                                sss = nan;
                                lon_sss = NaN;
                                lat_sss = NaN;
                            end
                            ...
                            if idata_type == 5 && itime_cnt <= datenum(2013,12,27)
                            ...
                            if idata_type == 7 && itime_cnt <= datenum(2018,11,01)
                            ...
                            elseif idata_type == 7 && itime_cnt > datenum(2018,11,01)
                                sss = nan;
                                lon_sss = NaN;
                                lat_sss = NaN;
                            ...
                            if idata_type == 8 && itime_cnt <= datenum(2013,12,27)
                            ...
                            elseif idata_type == 8 && itime_cnt > datenum(2013,12,27)
                                sss = nan;
                                lon_sss = NaN;
                                lat_sss = NaN;
                            end
                            ...
                            for nn = 1: nprof
                                ...
                                if any(~isnan(platform_irange)) && any(~isnan(JULD_irange)) && bin_data == 0
                                    platform_repeat = ...
                                ...
                                if Ln >= 1 && ismember(rep_prof,[0,1])
                                    ...
                                    if bin_data == 0
                                        ...
                                        JULD_irange (1,ncount) = JULD(nn);
                                        
                                        PRES_irange (xROW,ncount) = PRES(:,nn);
                                        SALT_irange (xROW,ncount) = SALT(:,nn);
                                    ...
                                    elseif bin_data == 1
                                        ...
                                        PRES_irange (1,ncount) = PRES(nn);
                                        SALT_irange (1,ncount) = SALT(nn);
                                        TEMP_irange (1,ncount) = TEMP(nn);
                                        
                                        clear a b
                                    end % interpolate profile in the vertical (only non binned data)
                                end
            ...
            %% Remove empty floats (*optimize space) -- only necessary to apply to non binned data
            ...
            elseif bin_data == 1
                ...
                ind = find(all(isnan(lon_irange),1));
