...
folder_figs = [folder_figs basin_str '/'...
    REF_str '/' bin_str '/' flag_str '/' region '/'];
if fg_save == 1
...
if make_plot == 1
    ...
    % %     a_beta = X1.sss_irange(:);
    % %     b_beta = X2.sss_irange(:);
    % %     c_beta = X3.sss_irange(:);
    % %     d_beta = X4.sss_irange(:);
    ...
    %* Exclude SSS data outside realistic values (min/max in each basin) 
    %* or refenrence data with nan
    SSS_min = 0;
    SSS_max = 20;
    
    ind1 = a_beta <= SSS_min | a_beta> SSS_max | isnan(X1.SALT_ref(:));
    ind2 = b_beta <= SSS_min | b_beta> SSS_max | isnan(X2.SALT_ref(:));
    ind3 = c_beta <= SSS_min | c_beta> SSS_max | isnan(X3.SALT_ref(:));
    ind4 = d_beta <= SSS_min | d_beta> SSS_max | isnan(X4.SALT_ref(:));
...
if make_plot == 1
    ...
    % index (ind) elements to exclude from stats (see previous section)
    a_beta(ind1) = NaN;
    b_beta(ind2) = NaN;
    c_beta(ind3) = NaN;
    d_beta(ind4) = NaN;
...
%* ============================
%* Do not include data with zeros or refenrence data with nan (done in
...
numb = all(isnan(c_beta));

if ~isempty(c_beta) && numb == 0
    [X3_mean,X3_median,X3_std,X3_stdRobust,X3_min,X3_max,...
