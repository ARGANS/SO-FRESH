...
%
% Description
% Analyses of Ref floats as provided by BEC.
...
%
%
% current version: v2r2 (2020/03/05) - Update plots and do some debugging
% (e.g. taylor diagram)
%
% History
%
...
%
% [2] Creation of snipet (functions) to make Ref vertical interpolations
...
iregion = 3;    % [5] ALL-BALTIC; see top of the script for other options
iyear = 2011:2013;
imonth = 1:12;
...
folder_colocations = [path_root...
    'SSS/Baltic/BEC/Validation/indata/SDN/Colocations/' flag_str '/cat_files/' ];

% ==========================


% filename Ref BEC structure: SDN_20110101_20110110.nc
ndays = 9; % number of days contained in each BEC Ref file


folder_figs = [folder_figs basin_str '/SDN/' flag_str '/' region '/'];
...
%% [2.2.1] SCATTERPLOTS: ALL-SSS MEAN at ALL-float location
% Compare each product to Salinity ref [0-10 m] average
Smin = 5;
Smax = 8;
...
if make_plot == 1
    ...
    Y_beta = sss_monthly_median1;
    X_beta = SALT_ref_monthly_median1;
    Z_beta = sss_monthly_median1;
...
%     Y_beta1 (Y_beta1 > Smax) = NaN;
    % ==========================
    figure    
    h1 = scatter(X_beta,Y_beta,[],Z_beta,'filled','MarkerEdgeColor','k'); hold on
    colormap(jet)
    
    ch = colorbar;
    ylabel(ch,'SSS (psu)','fontsize',18)
    ...
    fg_name = ['SDN_'...
        datestr(time_line(1),'yyyymmdd') '_'...
        datestr(time_line(end),'yyyymmdd') '_SCATTER_SSS_'...
        region  '_R' num2str(r) 'KM.'  fg_format];
