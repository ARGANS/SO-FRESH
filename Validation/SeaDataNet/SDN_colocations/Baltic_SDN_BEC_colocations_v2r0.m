function [vars_out] = Baltic_SDN_BEC_colocations_v2r0(iyear,imonth,iregion,idata_type,varargin)
%
%
% Syntax (function):
...
%% Flags script
run_test = 1;    % flag to run script (not to save figures);
...
elseif flag_on == 1
    folder_out = [path_root...
        'SSS/' basin_str...
        '/BEC/Validation/indata/SDN/Colocations/' flag_str '/monthly/'...
        version_str '/' region '/'];
end
            ...
            for dd = 1: length(idays)
                ...
                if ismember(itime_start,time_number) == 1
                    ...
                    elseif strcmpi(region,'ALL') % all-Arctic region stats
                        ind_reg_01 = find(lon_alfa >= 15 & lon_alfa <= 18 & lat_alfa > 53 & lat_alfa <= 58);
                        ind_reg_02 = find(lon_alfa >= 18 & lon_alfa <= 23 & lat_alfa > 60 & lat_alfa <= 64);
                        ind_reg_03 = find(lon_alfa >= 23 & lon_alfa <= 26 & lat_alfa > 58 & lat_alfa <= 62);
                        ind_reg_04 = find(lon_alfa >= 18 & lon_alfa <= 23 & lat_alfa > 56 & lat_alfa <= 59);
                       
                        % get indices of all locations
                        ind_reg_00 = vertcat(ind_reg_01, ind_reg_02, ind_reg_03, ind_reg_04);
                        ...
                        %* compare stats at regional level. New code does
                        %* include point [updated 1/07/2020 after PM#04]
