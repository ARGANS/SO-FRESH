function [vars_out] = Baltic_SDN_BEC_colocations_v3r0(iyear,imonth,iregion,idata_type,varargin)
%
%
% Syntax (function):
...
%
%
% current version: v3r0 (2020/07/15) -
%         [1] Center Colocation at the satellite grid;
%         [2] Bin in situ observations (REF) at the nearest satellite
%         grid-node
%
% History
% v3r0 (2020/07/15) - include feedback given at project PM#04 - see method in project report (PVR)
% v2r0 (2020/03/18) - To convert script into colocation function.
...
if run_test == 1
    clc
    warning ('Baltic_SDN_BEC_colocation.m is in TEST MODE')
...
folder_out = [path_root...
    ...
    bin_str '/' flag_str '/monthly/' version_str '/' region '/'];
...
% if bin_data = 0, use orginal the SDN dataset
if bin_data == 0
...
%     
%     % save Arctic grid
%     folder_grid = (['/Volumes/Rogue/Data/SSS/' basin_str '/BEC/' basin_str '_grid/']);
...
% 
% 
% [lon_beta,lat_beta] = meshgrid(lon,lat);
% 
% %* homogenize grid (global product to Baltic+)
% [SALT_beta] = griddata(lon_beta,lat_beta,SALT,lon_sss,lat_sss);
% [TEMP_beta] = griddata(lon_beta,lat_beta,TEMP,lon_sss,lat_sss);
% [PRES_beta] = griddata(lon_beta,lat_beta,PRES,lon_sss,lat_sss);
% ========================
...
for yy = 1:length(iyear)
    ...
    for mm = 1:length(imonth)
        ...
        % =================================
        fn_out = [folder_out basin_str '_' region '_'...
            REF_str '_R' r_str '_'...
        ...
        if fn_out_exist ~= 2 || run_test == 1
            ...
            for dd = 1: length(idays)
                ...
                itime_cnt = datenum(iYEAR,iMONTH,iDAY); % central date
                
                itime_start = itime_cnt-4;
                itime_end = itime_cnt+4;
                
                iTIME = [itime_start itime_end]; % keep time in vector
                
                %* >>>>>>>>>>>>>>>>>>>>
                %* load binned data as created using mat script 
                %*(for more info, type Baltic_SDN_BEC_binning_v1r0.m)
                ...
                if bin_data == 1
                    ...
                    folder_in = [path_root...
                        ...
                        REF_str '_mat' '/BINNED/' flag_str '/monthly/'];
                   
                    % load SDN-matlab file [fn]
                    fn = (['Baltic_' REF_str '_BINNED_25KM_' ...
                    ...
                    % in case bin data does not exist make an empty
                    % colocation file
                    if fn_exist == 2
                        TT = load(fn_in);
                        ...
                        SALT = TT.SALT_bin_median;
                        TEMP = TT.TEMP_bin_median;
                        PRES = TT.PRES_bin;
                        
                        count_bin = TT.count_bin;
                        
                        platform_type = nan(size(lon));
                        platform_ID = nan(size(lon));
                        time_cnt = TT.time_cnt;
                        
                        if ~isequal(size(time_cnt),size(lon))
                           time_cnt = ones(size(lon)).*time_cnt(1); 
                        end
                        
                        %* this is redundant. But original script 
                        %* (with no binning data) used time_number
                        time_number = time_cnt; 
                        
                        % ==========================
                        %* remove bin with zero counts
                        ind = count_bin == 0;
                ...
                if ismember(itime_cnt,time_number) == 1
                    
                    ind = time_number == itime_cnt;
                    ...
                    if strcmpi(region,'ARB') % North Atlantic
                        ...
                        ind_reg = lon_alfa >= 15 & lon_alfa <= 18  &...
                    ...
                    elseif strcmpi(region,'ALL') % all-Baltic region stats
                        
                        %* === Note ====
                        %* the 'old' code was including all points in the
                        %* baltic. This might be bring misalignment to
                        %* compare stats at regional level. New code only does
                        %* include points within each subregion 
                        %* [updated 1/07/2020 after PM#04]
                        ...
                        ind_reg_00 = unique(ind_reg_00);
                        
                        % convert ind_reg_00 to a logical index (ie. avoid
                        % bugs downstream code)
                        ind_reg = zeros(size(lon_alfa));
                        ind_reg(ind_reg_00) = 1;
                        
                    elseif strcmpi(region,'ALL_OLD') % ALL region stats using all points in Baltic 
                        
                        %# all code computing ALL Baltic stats 
                        %* (includes all points in basin)
                    ...
                    if ~isempty(lon_alfa) && ~isempty(lat_alfa) && sum(any(SALT_alfa))~=0
                        % Cases with more than one float (profile) in one
                        % file
                        if bin_data == 0
                        ...
                        if idata_type == 4 && itime_cnt >= datenum(2011,02,04) && itime_cnt <= datenum(2013,12,27)
                            data_type = idata_type; % Baltic+ (SSS-BEC NM v1.0)
                            
                            [TT] = rd_smos_L4_BEC_v1r3(itime_cnt,ibasin,data_type);
                        ...
                        elseif idata_type == 4 && (itime_cnt < datenum(2011,02,04) || itime_cnt > datenum(2019,08,31))
                            sss = nan;
                            lon_sss = NaN;
                            lat_sss = NaN;
                        end
                        ...
                        if idata_type == 5 && itime_cnt >= datenum(2011,02,04) && itime_cnt <= datenum(2013,12,27)
                        ...
                        elseif idata_type == 5 && (itime_cnt < datenum(2011,02,04) || itime_cnt > datenum(2019,08,31))
                            sss = nan;
                            lon_sss = NaN;
                            lat_sss = NaN;
                        end
                        ...
                        if idata_type == 7 && (itime_cnt >= datenum(2011,01,01) && itime_cnt <= datenum(2018,11,01))
                            
                            plot_cci_ex = 0;
                            cci_product = 1; % [1] '7-days', or [2] '30-days' product
                            [TT] = rd_sss_cci(itime_cnt,ibasin,cci_product,plot_cci_ex);
                        ...
                        elseif idata_type == 7 && (itime_cnt < datenum(2011,01,01) || itime_cnt > datenum(2018,11,01))
                            sss = nan;
                            lon_sss = NaN;
                            lat_sss = NaN;
                        ...
                        if idata_type == 8 && itime_cnt >= datenum(2011,01,01) && itime_cnt <= datenum(2013,12,16)
                            
                            grid2baltic = 0; % grid model output to Baltic grid
                            [YY,MM,DD] = datevec(itime_cnt);
                            
                            disp(['model at: ' datestr(itime_cnt)])
                        ...
                        elseif idata_type == 8 && (itime_cnt < datenum(2011,01,01) || itime_cnt > datenum(2013,12,27))
                            sss = nan;
                            lon_sss = NaN;
                            lat_sss = NaN;
                        end
                        ...
                        for nn = 1: nprof
                            ...
                            if Ln >= 1
                                ...
                                if bin_data == 0
                                    ...
                                    PRES_irange (1,ncount) = PRES_alfa(nn);
                                    SALT_irange (1,ncount) = SALT_alfa(nn);
                                    TEMP_irange (1,ncount) = TEMP_alfa(nn);
