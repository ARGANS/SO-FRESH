...
if plot_time_series == 1
    ...
    for nn = 1 : length(param_model_str)
        ...
        eval(['h2 = boundedline(TIME_model,'...
            ...
            ['''transparency'''] ',' num2str(shade_transparent) ');']);
        hold on
        ...
        title(['Regional averaged (mean) Baltic ' param_model_str2{nn}],...
            'fontsize',24);
        ylabel([param_model_str2{nn} ' (' param_units{nn} ')' ],...
            'fontsize',24);
    ...
    for pp = 1 %: length(SREG_str) % loop each study region
        ...
        for nn = 1 : length(param_model_str) % loop each parameter
            ...
            eval(['boxplot(' param_model_str{nn}...
                '_irange(:,:,' num2str(pp) '));']); hold on
            
            eval(['h1 = plot(' param_model_str{nn}...
                '_median_sreg(:,' num2str(pp) '),'...
                ['''ok'''] ',' ['''linewidth'''] ',2);'])
            eval(['h2 = plot(' param_model_str{nn}...
                '_mean_sreg(:,' num2str(pp) '),'...
                ['''--k'''] ',' ['''linewidth'''] ',2);'])
            ...
            box on
            ylabel([param_model_str{nn} ' (' param_units{nn} ')' ],...
                'fontsize',24);
...
[X1_mean,X1_median,X1_std,X1_min,X1_max,...
...
%% [6.2.2.2] Histogram Regional dSSS_02 = NEMO - NS
