...
%% [1] Study of the SSS model data (NEMO)
%% [1.1] Load model_data (snippet)
% this snipped uses function [data_model] = nemo_rd_BEC_Baltic_FUN (iyear,imonth,iday,ibasin,grid_output)

grid_output = 1; % grid nemo to SMOS-BEC grid [1]; or not [0]; 

Baltic_model_BEC_load_data;
%% [1.2] Plot monthly MAPS
...
%% [1.3] Model Regional study (variability within each study region)
% There are four Baltic+ study regions (see Baltic+ DUM, p. 28): [1] Arkona 
% Basin [ArB] (55°31'30.22"N, 16°16'15.06"E) [2] Bothian Sea [BOS] (61°55'50.19"N, 
% 19°14'46.24"E) [3] Gulf of Finland [GOF] (59°35'55.07"N, 23°07'27.96"E) [4] 
% Northern Baltic Proper [NBP] (57°36'10.05"N, 19°48'25.78"E)
...
r = 200; % km
%% [1.3.1] Plot with all the study regions in the Baltic as seen in DUM (p.28)

figure
plot_study_region(lon_sreg,lat_sreg,r); hold on

for nn = 1:length(SREG_str)
    h1(nn) = plot(lon_sreg(nn),lat_sreg(nn),'w.'); hold on
...
lg = legend(h1,SREG_str{:},'location','Southeast','fontsize',18);
...
title({['Baltic+ Study Regions ']...
    ['(Radius: ' num2str(r) ' km)']},'fontsize',24);
...
%% [2] Time series at each location (Plot stats)
...
%% [2.1] >> Time Series Mean
    ...
    for nn = 1 : length(param_model_str)
        ...
        title(['Regional averaged (mean) Baltic ' param_model_str2{nn}],'fontsize',24);
        ylabel([param_model_str2{nn} ' (' param_units{nn} ')' ], 'fontsize',24);
        
    end
    
    fg_name = [basin_str '_MEAN_NEMO_' time_str1 '_' time_str2 '_ALL'];
    
    % Save figure  - output -
    folder_this = [folder_figs 'model/ALL/'];
...
%% [2.2] >> Time Series Median [box plot at each study region]
    ...
    for pp = 1 %: length(SREG_str) % loop each study region
        ...
        fg_name = [basin_str '_MEADIAN_NEMO_' ...
            time_str1 '_' time_str2 '_' SREG_str2{pp}];
        
        % Save figure  - output -
        folder_this = [folder_figs 'model/REGIONAL/' ];
...
%
% =================
%% [4.2] dSSS = SSS-SMOS Regional stats
...
Nobs_NM_irange2 = squeeze(sum(~isnan(SSS_NM_irange2)));

SSS_NS_irange2 = reshape(SSS_NS_irange,a*b,c,d);
SSSe_NS_irange2 = reshape(SSSe_NS_irange,a*b,c,d);

Nobs_NS_irange2 = squeeze(sum(~isnan(SSS_NS_irange2)));
...
SSS_NM_irange_SEM = SSS_NS_irange_std./ (Nobs_NM_irange2.^0.5);
SSS_NS_irange_SEM = SSS_NS_irange_std./ (Nobs_NS_irange2.^0.5);
%% [5] dSSS = SSS-SMOS Time Series
%% [5.1] Regional SSS Time series Raw-series
%% [5.1.1] Regional SSS-Time series STD around the mean
...
for nn = 1:length(SREG_str)
    ...
    h3 = boundedline(TIME_model,SSS_model_mean_sreg(:,nn),...
        SSS_model_std_sreg(:,nn), 'b-',...
        'alpha','orientation','vert','transparency',0.3); hold on
    set(h3,'linewidth',2);
    ...
    title({['Regional averaged (STD around mean)'],...
        ['Baltic ' SREG_str2{nn}]},'fontsize',24);
    xlabel('time (months)','fontsize',24);
    ...
    % Save figure  - output -
    folder_this = [folder_figs 'NEMOvsSAT/TIMESERIES/SSS/'];
...
%% [5.1.2] Regional SSS-Time series SEM around the mean

for nn = 1:length(SREG_str)
    
    X1 = time_range';
    Y1 = SSS_NM_irange_mean(:,nn);
    Z1 = SSS_NM_irange_SEM(:,nn);
    
    X2 = time_range';
    Y2 = SSS_NS_irange_mean(:,nn);
    Z2 = SSS_NS_irange_SEM(:,nn);
    ...
    set(gca,'xtick',TIME_model(1:2:end))
    datetick('x','mm','keepticks')
    title({['Regional averaged (SEM around mean)'],...
        ['Baltic ' SREG_str2{nn}]},'fontsize',24);
    xlabel('time (months)','fontsize',24);
    ylabel([ 'SSS (psu)' ], 'fontsize',24);
    ...
    % Save figure  - output -
    folder_this = [folder_figs 'NEMOvsSAT/TIMESERIES/SSS/SEM/'];
...
nobs_NM_alfa = sum(~isnan(dSSS_NM_alfa),1);
nobs_NS_alfa = sum(~isnan(dSSS_NS_alfa),1);


dSSS_NM_mean = nanmean(dSSS_NM_alfa,1);
dSSS_NM_std = nanstd(dSSS_NM_alfa,1);
dSSS_NM_sem = dSSS_NM_std ./ (nobs_NM_alfa.^0.5);


dSSS_NS_mean = nanmean(dSSS_NS_alfa,1);
dSSS_NS_std = nanstd(dSSS_NS_alfa,1);
dSSS_NS_sem = dSSS_NS_std ./ (nobs_NS_alfa.^0.5);
...
% ====================================
%% [5.2.1] Plot std around average (mean) dSSS
...
lg = legend([h1(1);h2(1)],...
    {' SSS-BEC_{NM} - SSS_{NEMO}';' SSS-BEC_{NS} - SSS_{NEMO}' },...
    'fontsize',14);
set(lg,'Location','northwest')
...
title({['Regional averaged (std around mean) dSSS'], ['Baltic ALL']},'fontsize',24);
xlabel('time (months)','fontsize',24);
ylabel([ 'dSSS (psu)' ], 'fontsize',24);

fg_name = [basin_str '_dSSS-STD_NEMOvsSAT_ALL_'...
    time_str1 '_' time_str2];

% Save figure  - output -
folder_this = [folder_figs 'NEMOvsSAT/TIMESERIES/dSSS/STD/'];
...
%% [5.2.2] Plot SEM around average (mean) dSSS
...
set(gca,'xtick',TIME_model(1:2:end))
datetick('x','mm-yy','keepticks')
title({['Regional averaged (SEM around mean) dSSS'], ['Baltic ALL']},'fontsize',24);
xlabel('time (months)','fontsize',24);
ylabel([ 'dSSS (psu)' ], 'fontsize',24);

fg_name = [basin_str '_dSSS-SEM_NEMOvsSAT_ALL_'...
    time_str1 '_' time_str2];

% Save figure  - output -
folder_this = [folder_figs 'NEMOvsSAT/TIMESERIES/dSSS/SEM/'];
...
Nobs_dSSS_NM = squeeze(sum(~isnan(dSSS_NM_irange))); % number observations

dSSS_NM_irange_SEM = dSSS_NM_irange_std./(Nobs_dSSS_NM.^0.5);


dSSS_NS_irange_mean = squeeze(nanmean(dSSS_NS_irange,1));
dSSS_NS_irange_std = squeeze(nanstd(dSSS_NS_irange,1));

Nobs_dSSS_NS = squeeze(sum(~isnan(dSSS_NS_irange))); % number observations

dSSS_NS_irange_SEM = dSSS_NS_irange_std./(Nobs_dSSS_NS.^0.5);
%% [5.4] Plot dSSS (Sat - Model)
%% [5.4.1] [USE Standard deviation around the mean]
...
for nn = 1:length(SREG_str)
    ...
    set(gca,'xtick',TIME_model(1:2:end))
    datetick('x','mm/yy','keepticks')
    title({['Regional averaged (std around mean) dSSS '],...
        ['Baltic ' SREG_str2{nn}]},'fontsize',24);
    xlabel('time (months)','fontsize',24);
    ylabel([ 'dSSS (psu)' ], 'fontsize',24);
    
    fg_name = [basin_str '_dSSS-STD_NEMOvsSAT_' SREG_str2{nn} '_'...
        time_str1 '_' time_str2];
...
%% [5.4.2] [USE Standard Error of the mean [SEM] around the mean]
...
for nn = 1:length(SREG_str)
    ...
    X1 = TIME_model';
    Y1 = dSSS_NM_irange_mean(:,nn);
    Z1 = dSSS_NS_irange_SEM(:,nn);
    
    X2 = TIME_model';
    Y2 = dSSS_NS_irange_mean(:,nn);
    Z2 = dSSS_NS_irange_SEM(:,nn);
    ...
    set(gca,'xtick',TIME_model(1:2:end))
    datetick('x','mm/yy','keepticks')
    title({['Regional averaged (SEM around mean) dSSS '],...
        ['Baltic ' SREG_str2{nn}]},'fontsize',24);
    xlabel('time (months)','fontsize',24);
    ylabel([ 'dSSS (psu)' ], 'fontsize',24);
    
    fg_name = [basin_str '_dSSS-SEM_NEMOvsSAT_' SREG_str2{nn} '_'...
        time_str1 '_' time_str2];
...
%% [6] Salinity Histograms

Smax = 35;  % (psu)
Smin = 0;   % (psu)

nbins = 100; % number of histogram-bins
sss_groups = linspace(Smin,Smax,nbins);
%% [6.1] Histogram SSS ALL Baltic
%% [6.1.1] Histogram ALL Baltic SSS NEMO (model)

% input the variable to make histogram
X1 = SSS_model;
X1_str = {'SSS_{NEMO}'};
...
figure
histogram(X1(:),nbins)
[N,edges] = histcounts(X1(:),nbins);
box on
set(gca,'XMinorTick','on','YMinorTick','on')

ylim=get(gca,'ylim');
xlim=get(gca,'xlim');
h1 = text(xlim(2)*.5,ylim(2)*.5,sprintf('%s\n',STATS_str{:}),'fontsize',18);

grid minor

set(gca,'FontSize',20)

title({[X1_str{:} ],['Baltic ' 'ALL']},'fontsize',24);
xlabel('Salintiy (psu)','fontsize',24);
ylabel([ 'Frequency' ], 'fontsize',24);

fg_name = [basin_str '_' X1_str{:} '_HIST_'...
    time_str1 '_' time_str2 '_ALL'];

% Save figure  - output -
folder_this = [folder_figs 'NEMOvsSAT/HISTOGRAMS/SSS/'];
...
%% [6.1.2] Histogram ALL Baltic SSS SMOS (satellite)
%% [6.1.2.1] Histogram ALL Baltic SSS_NM (Nominal Sampling)
% input the variable to make histogram
...
%% [6.1.2.2] Histogram ALL Baltic SSS_NS (Nodal Sampling)
% input the variable to make histogram
...
%% [6.1.3] Regional Histogram SSS
%% [6.1.3.1] Histogram Regional SSS NEMO (model)

for nn = 1:length(SREG_str)
% input the variable to make histogram
X1 = SSS_model_irange(:,:,nn);
X1_str = 'SSS_{NEMO}';
...
title({[X1_str], ['Baltic ' SREG_str2{nn}]},'fontsize',24);
...
fg_name = [basin_str '_' X1_str '_HIST_'...
    time_str1 '_' time_str2 '_' SREG_str2{nn}];
...
%% [6.2.2] Histogram Regional SSS SMOS (satellite)

[a,b,c,d] = size(SSS_NM_irange);
SSS_NM_irange2 = reshape(SSS_NM_irange,a*b,c,d);
SSS_NS_irange2 = reshape(SSS_NS_irange,a*b,c,d);
%% [6.1.2.1] Histogram Regional SSS Nominal Sampling (NM)

for nn = 1:length(SREG_str)
% input the variable to make histogram
X1 = SSS_NM_irange2(:,:,nn);
X1_str = 'SSS_{NM}';
...
% figure;
% dist = 'normal';
% histfit(X1(ind),nbins,dist)

ylim=get(gca,'ylim');
xlim=get(gca,'xlim');
h1 = text(xlim(2)*.5,ylim(2)*.5,sprintf('%s\n',STATS_str{:}),'fontsize',18);

set(h1,'HorizontalAlignment', 'right',...
...
%% [6.1.2.2] Histogram Regional SSS Nodal Sampling (NS)

for nn = 1:length(SREG_str)
% input the variable to make histogram
X1 = SSS_NS_irange2(:,:,nn);
X1_str = 'SSS_{NS}';
...
%% ========================================================================
%% [6.2] Histogram dSSS = MODEL - SATELLITE
%% [6.2.1.1] Histogram ALL Baltic dSSS_01 = NEMO - NM

% input the variable to make histogram
X1 = dSSS_NM;
X1_str = {'dSSS_{NEMO-NM}'};
...
% Save figure  - output -
folder_this = [folder_figs 'NEMOvsSAT/HISTOGRAMS/dSSS/'];
...
%% [6.2.1.2] Histogram ALL Baltic dSSS_02 = NEMO - NS
% input the variable to make histogram
...
%% [6.2.2] Regional Histogram dSSS
%% [6.2.2.1] Histogram Regional dSSS_01 = NEMO - NS

for nn = 1:length(SREG_str)
% input the variable to make histogram
X1 = dSSS_NM_irange(:,:,nn);
X1_str = 'SSS_{NEMO-NM}';
...
%% [6.2.2.2] Histogram Regional dSSS_02 = NEMO - NSx

for nn = 1:length(SREG_str)
% input the variable to make histogram
X1 = dSSS_NS_irange(:,:,nn);
X1_str = 'SSS_{NEMO-NS}';
...
%% [2.3] >> SMOS Time Series Mean (TO-CORRECT SECTION using SMOS)

SSS_NM_irange_mean_sreg = squeeze(nanmean(SSS_NM_irange_mean_month2,1));
SSS_NS_irange_mean_sreg = squeeze(nanmean(SSS_NS_irange_mean_month2,1));

SSS_NM_irange_std_sreg = squeeze(nanstd(SSS_NM_irange_mean_month2,1));
SSS_NS_irange_std_sreg = squeeze(nanstd(SSS_NS_irange_mean_month2,1));
...
h1 = plot(TIME_model,SSS_model_mean_sreg_ALL,'--k','linewidth',2); hold on
h2 = boundedline(TIME_model,SSS_model_mean_sreg,SSS_model_std_sreg_ALL,'alpha','orientation','vert','transparency',0.3);
...
h1 = plot(TIME_model,SSS_NS_irange_mean_sreg,'--k','linewidth',2); hold on

h2 = boundedline(ones(36,4).*TIME_model,SSS_NM_irange_mean_sreg,SSS_NM_irange_std_sreg,'alpha','orientation','vert','transparency',0.3);
